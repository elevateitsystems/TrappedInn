...
export default function PublicProfilePage() {
    ...
    return (
      <div className="min-h-dvh bg-background flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-muted-foreground mb-6">
          This username doesn't exist on {APP_NAME}.
        </p>
  ...
  const verificationLevel = (p.verificationLevel ??
    (p.verified ? "blue" : "none")) as VerificationLevel;
  ...
  const contactButtons = [
    p.phone &&
      cs.showPhone && { label: "Call", href: `tel:${p.phone}`, icon: Phone },
    p.smsNumber &&
      cs.showSms && {
        label: "Text",
        href: `sms:${p.smsNumber}`,
        icon: MessageSquare,
      },
    p.email &&
      cs.showEmail && { label: "Email", href: `mailto:${p.email}`, icon: Mail },
    p.website &&
      cs.showWebsite && {
        label: "Website",
        href: p.website,
    ...
    >
      ...
      <div className="w-full max-w-sm md:max-w-xl mx-auto">
        ...
        >
          ...
          {p.headerImageUrl ? (
            <div className="relative mb-10 md:mb-16">
              ...
              {/* Avatar centered */}
              <div className="absolute left-1/2 -bottom-12 md:-bottom-16 -translate-x-1/2 z-10">
                {profile?.avatarUrl ? (
                  <Image
                    ...
                    height={160}
                    priority
                    className="w-28 h-28 md:w-40 md:h-40 rounded-full object-cover shadow-lg border-4 border-background"
                  />
                ) : (
                  <div className="w-28 h-28 md:w-40 md:h-40 rounded-full gradient-primary flex items-center justify-center text-white font-display font-semibold text-3xl md:text-5xl border-4 border-background shadow-lg">
          ...
          <div className={`flex flex-col items-center text-center`}>
            {!p.headerImageUrl &&
              (profile?.avatarUrl ? (
                ...
                <div
                  className={`gradient-primary flex items-center justify-center text-white font-display font-semibold mb-4 ${layout === "hero" ? "w-28 h-28 md:w-36 md:h-36 rounded-3xl text-4xl md:text-5xl" : "w-20 h-20 md:w-28 md:h-28 rounded-full text-3xl md:text-4xl"}`}
                >
                  {profile?.displayName?.[0]?.toUpperCase() || "--"}
                </div>
              ))}
            ...
            {p.activeMode && (
              <div className="mt-4 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-medium mb-2">
            ...
            <div className="flex items-center gap-1.5">
              <h1
                className="text-2xl md:text-4xl font-display font-semibold"
            ...
            {profile?.bio && (
              <p className="text-sm md:text-base text-muted-foreground mt-3 leading-relaxed max-w-xs md:max-w-md">
                {profile?.bio}
              </p>
            )}
          </div>
        ...
        <div className="space-y-3">
          {profile?.links?.map(
            (link: { id: string; url: string; title: string }, i: number) => {
              ...
              return (
                <motion.a
                  ...
                  className={`flex items-center gap-4 w-full px-5 py-4 border card-glow hover:card-glow-hover hover:bg-accent transition-all duration-200 group ${buttonBg} ${buttonRadius}`}
                  style={{ color: textColor }}
                >
          ...
          >
            ...
            {leadDone ? (
              <p className="text-sm text-primary text-center py-2">
                Thanks! You'll be in touch soon.
              </p>
            ) : (
