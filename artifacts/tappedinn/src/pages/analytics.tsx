...
import { BarChart2, Eye, MousePointerClick, Wifi } from "lucide-react";
...
function StatBadge({ label, value, icon: Icon }: { label: string; value: number; icon: React.ElementType }) {
  return (
    <div className="p-4 rounded-xl border border-border bg-card flex items-center gap-3">
      ...
      <div>
        <p className="text-lg font-display font-semibold">{value.toLocaleString()}</p>
    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-muted-foreground mt-1 text-sm">How your profile is performing</p>
        </motion.div>
